...
public final class myProtocolDecoder {
	...
	private ComPackage rPacket = new ComPackage();
	public ComPackage RecPackage = new ComPackage();
	...
	private ArrayList<DecodeEventListener> Listeners = new ArrayList<DecodeEventListener>();

	private void publishEvent(DecodeEvent event) {
		for(DecodeEventListener listener : Listeners) {
			...
			case DECODE_STATE_GOT_DATA:
				...
				if(CalculateCRC.ComputeCRC8(rPacket.getCRCBuffer(), _rxlen, CRC_INIT) == rPacket.crc) {
					synchronized(new String("")) {//critical
						...
						DecodeEvent event = new DecodeEvent(RecPackage);
						publishEvent(event);
					}
				} else {
					for(DecodeEventListener listener : Listeners) {
	...
	public void addDecodeListener(DecodeEventListener listener) {
